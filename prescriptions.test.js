...
describe("getTotalCost()", () => {
  ...
  // NOTE: These tests will fail - why? its not checking for negatiive values and will perfom the calculation
...
describe("applyDiscount()", () => {
  it("applies the correct percentage discount", () => {
    expect(applyDiscount(100, 20)).toBe(80);
  });
  it ("returns same cost for 0% discout", () => {
    expect(applyDiscount(100, 0)).toBe(100);
  });
});

  describe("applyDiscount()", () => {
    it("applies coupon discount when", () =>{
      expect(applyCoupon(50,true)).toBe(40);
    });
    it("returns the same cost when coupon is not present", () => {
